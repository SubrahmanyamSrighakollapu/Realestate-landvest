...
const Gallery = ({ onNext, onPrevious, currentStep, projectData }) => {
  ...
  const handleSaveAndContinue = async () => {
    ...
    try {
      ...
      const response = await projectService.updateImages(data);
      if (response.success) {
        toastService.success('Gallery images uploaded successfully!');
        onNext();
      }
    } catch (error) {
  ...
  return (
    <div>
      ...
      <p style={{
        ...
        Upload multiple gallery images to show in the site
      </p>
      ...
      >
        <input
          ...
          type="file"
          accept="image/*"
          multiple // allow multiple gallery images
        ...
        >
          {images.length > 0 ? 'Add More Images' : 'Choose Files'}
        </button>
      </div>
