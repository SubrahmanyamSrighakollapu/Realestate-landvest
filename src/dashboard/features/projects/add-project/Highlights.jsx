...
const Highlights = ({ onNext, onPrevious, currentStep, projectData, setProjectData }) => {
  ...
  const handleAddHighlight = () => {
    ...
    setCurrentHighlight({ title: '', description: '' });
  };
  ...
  const handleSaveAndContinue = async () => {
    if (highlights.length === 0) {
      toastService.error('Please add at least one highlight');
      return;
    }
    ...
    try {
      ...
      const response = await projectService.updateHighlights(payload);
      if (response.success) {
        toastService.success('Highlights updated successfully!');
        onNext();
      }
    } catch (error) {
      toastService.error(error.response?.data?.message || 'Failed to update highlights');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div>
      ...
      <p style={{
        ...
        Add key features and highlights of your project
      </p>
      ...
      <div style={{
        ...
        <h3 style={{ fontSize: '16px', fontWeight: '600', marginBottom: '16px' }}>Add New Highlight</h3>
      ...
      {highlights.length > 0 && (
        <div style={{ marginBottom: '32px' }}>
          <h3 style={{ fontSize: '16px', fontWeight: '600', marginBottom: '16px' }}>Added Highlights</h3>
